...
// Explicit inline Card component layout inside Profile to prevent crashes
...
useEffect(() => {
  ...
  // 1. Safe parsing of the logged-in session profile matrix
const joinedDate =
...
const isLoggedIn = !!user;
  // 2. Fetch entire system state library entries safely
  // Lifetime metrics parsed seamlessly
  const totalMovies =
  isLoggedIn
    ? getTotalMoviesCompleted(library)
    : 0;

const totalSeries =
  isLoggedIn
    ? getTotalSeriesCompleted(library)
    : 0;

const totalHours =
  isLoggedIn
    ? getTotalHoursWatched(library)
    : 0;

const highestStreak =
  isLoggedIn
    ? getHighestStreak(library)
    : 0;

  // Monthly tracking benchmarks
  const monthMovies =
  isLoggedIn
    ? getMoviesCompletedThisMonth(library)
    : 0;

const monthSeries =
  isLoggedIn
    ? getSeriesCompletedThisMonth(library)
    : 0;

const monthHours =
  isLoggedIn
    ? getHoursWatchedThisMonth(library)
    : 0;

const currentStreak =
  isLoggedIn
    ? getCurrentStreak(library)
    : 0;
  const weekDays = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
  const [selectedYear, setSelectedYear] = useState(new Date().getFullYear());

  // 3. Dynamic Heatmap tracking mapped securely via utility parameters
  const heatMap =
  isLoggedIn
    ? (
        getHeatMap(
    ...
    : {};
  // --- Real GitHub Grid Rendering Engine ---
  const generateGitHubGrid = () => {
    ...
    for (let w = 0; w < totalWeeks; w++) {
      ...
      for (let d = 0; d < 7; d++) {
        ...
        const active = heatMap[dateKey]; // Checks activity counter mapping safely
        const currentMonthName = currentTrackDate.toLocaleString("en-US", { month: "short" });

        if (
          currentMonthName !== lastRenderedMonth && 
          (d === 0 || currentTrackDate.getDate() <= 7) &&
          (w - lastRenderedWeekIndex >= 3)
        ) {
...
>
  {isLoggedIn
    ...
    : "?"}
            ...
            <p className="mt-2.5 font-bold tracking-wider text-xs uppercase bg-cyan-500/10 text-cyan-400 px-3 py-1 rounded-full border border-cyan-500/20">
              {isLoggedIn ? "WatchVerse Explorer" : "Sign In To Track Progress"}
            </p>
        ...
        <div className="flex justify-end">
          <select
            ...
            className="bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-sm text-slate-200 outline-none cursor-pointer hover:border-slate-500 transition-colors"
          >
        ...
        {/* --- PREMIUM DYNAMIC GITHUB PROFILE GRID (HEATMAP) --- */}
        <section className="space-y-3 pt-2">
          <div
            ref={scrollGraphRef}
            onWheel={handleGraphWheel}
            className="rounded-2xl border border-slate-800/80 bg-gradient-to-b from-[#0d1117] to-[#090d16] p-6 overflow-x-auto scroll-smooth [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]"
          >
            <div className="min-w-[1020px] flex flex-col relative">
              ...
              <div className="relative h-6 pl-9 w-full mb-2 select-none">
                {monthLabels.map((month, mIdx) => (
                  <span
                    key={mIdx}
                    className="absolute text-[11px] font-bold text-slate-400/80 tracking-wide transition-colors hover:text-white"
                    style={{ left: `${36 + month.weekIndex * 18}px` }}
                  >
                    {month.name}
                  </span>
                ))}
              ...
              {/* CORE METADATA GRAPH MATRIX */}
              <div className="flex items-start">
                <div className="flex flex-col gap-[5px] pr-3 justify-start pt-[1.5px] text-right w-6 select-none shrink-0">
                  {weekDays.map((day) => (
                    <span key={day} className="text-[10px] font-bold text-slate-500 h-[13px] leading-none flex items-center justify-end">
                      {day}
                    </span>
                  ))}
                </div>
                ...
                <div className="flex gap-[5px]">
                  {weeksArray.map((week, wIdx) => (
                    <div key={wIdx} className="flex flex-col gap-[5px] w-[13px]">
                      {week.map((day, dIdx) => {
                        const count = day.active || 0;
                        let colorClass = "bg-slate-800/40 hover:bg-slate-700"; // No activity style
                        ...
                        return (
                          <div
                            key={dIdx}
                            title={`${day.dateKey} : ${count} Activity registered`}
                            className={`h-[13px] w-[13px] rounded-[2px] transition-all duration-200 cursor-pointer ${colorClass}`}
                          />
                        );
                      })}
                    </div>
